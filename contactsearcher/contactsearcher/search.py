...
def search_for_email_given_job(job_description: str, contacts: str) -> List[List[str]]:
    ...
    for item in contacts.splitlines(): # iterate through the file, parsing it line by line
    ...
    for contact in contact_set: #iterates through set
        if job_description in contact: # checks to see if the job description is mentioned in the particular string in the set 
